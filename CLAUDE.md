...
- **스캔이 만드는 사람 읽는 문장은 완성된 문자열로 담지 않는다.** `ScanProblem` 은
  `keyed(area, target, i18n키, 인자)` 로 담고 화면·REST 가 보는 사람의 언어로 그린다.
  스캔 스레드에는 로케일이 없고 결과 한 벌을 두 언어가 함께 보기 때문이다 —
  v1.0.0~v1.4.0 은 영어 화면의 "확인 불가" 이유가 한글이었다(실측 41번).
  예외 텍스트만 `raw()` 로 그대로 담는다. 인자는 하나까지(2-arg 오버로드가 없다).
