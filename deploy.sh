#!/usr/bin/env bash
# 실행 중인 Jira(UPM)에 jar를 업로드한다. atlas-run은 쓰지 않는다.
#
#   JIRA_BASE=http://host:port JIRA_USER=admin JIRA_PASS='...' ./deploy.sh
#
...
curl -s -u "$USER:$PASS" \
    ...
    -F "plugin=@$JAR" \
    "$BASE/rest/plugins/1.0/?token=$TOKEN" > /tmp/upm-upload-janitor.json
